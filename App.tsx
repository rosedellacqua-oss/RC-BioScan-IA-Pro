...
// UI Components
const Header: React.FC<{ onAdmin?: () => void; onSignOut?: () => void; isAdmin?: boolean }> = ({ onAdmin, onSignOut, isAdmin }) => (
    ...
    <div className="flex items-center gap-2">
      {isAdmin && onAdmin && (
        <button
          id="header-admin-btn"
          onClick={onAdmin}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 hover:border-amber-500/50 text-amber-400 text-xs font-bold transition-all"
        >
      ...
      {onSignOut && (
        <button
          ...
          onClick={onSignOut}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-800/60 border border-slate-700 hover:border-red-500/40 hover:text-red-400 text-slate-400 text-xs font-bold transition-all"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
